...
function App() {
  ...
  const handleCompanyLogin = (safeData, fullData) => {
    setLoggedCompany(fullData || safeData);
    localStorage.setItem("loggedCompany", JSON.stringify(safeData));
  };

  // Re-hydrate the full company data (like massive logo/signature) on refresh
  // without bloating the localStorage.
  useEffect(() => {
    if (loggedCompany?.id && loggedCompany.logo === undefined) {
      const fetchFullData = async () => {
        try {
          const docRef = doc(db, "companies", loggedCompany.id);
          const snap = await getDoc(docRef);
          if (snap.exists()) {
            setLoggedCompany((prev) => ({
              ...prev,
              ...snap.data(),
              id: snap.id,
              logoFetched: true,
          ...
          console.error(
            "Error fetching full company data for session hydrate",
            e,
          );
        }
      };
  ...
  }, [loggedCompany?.id, loggedCompany?.logo]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      doc(db, "companies", loggedCompany.id),
      (docSnapshot) => {
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/vendor")) {
      ...
      path.startsWith("/add-party") ||
      path.startsWith("/edit-party")
    ) {
    ...
    const companySuffix =
      loggedCompany && !path.startsWith("/vendor")
  ...
  if (!loggedCompany) {
    ...
    return (
      <Routes>
        <Route
          path="*"
          element={<CompanyLogin onLogin={handleCompanyLogin} />}
        />
      </Routes>
    );
  }
    ...
    <div className="app-container">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-logout-container">
          <button className="sidebar-logout-btn" onClick={handleCompanyLogout}>
            <LogOut size={18} />
            Sign Out
          </button>
        </div>
